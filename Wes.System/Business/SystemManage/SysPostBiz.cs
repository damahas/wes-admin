...
namespace Wes.Business
{
    public class SysPostBiz : ISysPostBiz
    {
        ...
        {
            return new ResultData<SysPostEntity>(_sysPostService.GetById(id));
        }

        public RowData<SysPostEntity> GetList(ParamData<PostParam> param)
        {
            int total = 0;
            RowData<SysPostEntity> result = new RowData<SysPostEntity>(_sysPostService.GetList(param, out total));
            result.total = total;
            return result;
        }
