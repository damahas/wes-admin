...
namespace Wes.Business
{
    public class SysMessageBiz : ISysMessageBiz
    {
        ...
        {
            return new ResultData<SysMessageEntity>(_sysMessageService.GetById(id));
        }

        public ResultData<List<SysMessageEntity>> GetAll()
        {
            return new ResultData<List<SysMessageEntity>>(_sysMessageService.GetAll());
        }

        public RowData<SysMessageEntity> GetList(ParamData<MessageParam> param)
        {
            int total = 0;
            RowData<SysMessageEntity> result = new RowData<SysMessageEntity>(_sysMessageService.GetList(param, out total));
            result.total = total;
            return result;
        }
