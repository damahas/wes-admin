...
namespace Wes.Business
{
    public class SysLogBiz : ISysLogBiz
    {
        ...
        {
            int total = 0;
            RowData<SysLoginLogEntity> result = new RowData<SysLoginLogEntity>(_sysLoginLogService.GetList(param, out total));
            result.total = total;
            return result;
        }
        ...
        {
            int total = 0;
            RowData<SysOperLogEntity> result = new RowData<SysOperLogEntity>(_sysOperLogService.GetList(param, out total));
            result.total = total;
            return result;
        }
