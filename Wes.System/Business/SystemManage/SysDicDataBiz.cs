...
namespace Wes.Business
{
    public class SysDicDataBiz : ISysDicDataBiz
    {
        ...
        {
            int total = 0;
            RowData<SysDicDataEntity> result = new RowData<SysDicDataEntity>(_sysDicDataService.GetList(param, out total));
            result.total = total;
            return result;
        }
        ...
        {
            return new ResultData<SysDicDataEntity>(_sysDicDataService.GetById(id));
        }
        ...
        {
            var dics = CacheFactory.Cache.GetCache<List<SysDicDataEntity>>($"{CacheKey.Dic}{dicType}");
            if (dics != null)
            {
                return dics;
            }
        ...
        private List<SysDicDataEntity> Refresh(string dicType)
        {
