...
namespace Wes.Business
{
    public class SysCodeRuleBiz : ISysCodeRuleBiz
    {
        ...
        public ResultData<CodeRuleInfo> GetById(long id)
        {
            ...
            CodeRuleInfo result = codeRule.ToEntityCopy<SysCodeRuleEntity, CodeRuleInfo>();
        ...
        {
            return new ResultData<List<SysCodeRuleEntity>>(_sysCodeRuleService.GetAll());
        }

        public RowData<SysCodeRuleEntity> GetList(ParamData<CodeRuleParam> param)
        {
            int total = 0;
            RowData<SysCodeRuleEntity> result = new RowData<SysCodeRuleEntity>(_sysCodeRuleService.GetList(param, out total));
            result.total = total;
            return result;
        }
        ...
        public ReturnData Save(CodeRuleInfo model)
        {
            ...
            try
            {
                if (!_sysCodeRuleService.Save(model.ToEntityCopy<CodeRuleInfo, SysCodeRuleEntity>(), client))
                {
                    client.Ado.RollbackTran();
                ...
                foreach (var item in model.Parts)
                {
                    ...
                    // 这里处理重置问题
                    SysCodeRulePartEntity oldPart = null;
        ...
        private bool PartsToCode(ref List<SysCodeRulePartEntity> parts, out string code, out string errMsg)
        {
            errMsg = "";
            code = "";
        ...
        private DateTime GetNextResetTime(SysCodeRulePartEntity part)
        {
        ...
        private string ResetPartIndex(SysCodeRulePartEntity part)
        {
