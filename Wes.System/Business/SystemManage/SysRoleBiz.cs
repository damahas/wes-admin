...
namespace Wes.Business
{
    public class SysRoleBiz : ISysRoleBiz
    {
        ...
        {
            return new ResultData<SysRoleEntity>(_sysRoleService.GetById(id));
        }

        public RowData<SysRoleEntity> GetList(ParamData<RoleParam> param)
        {
            int total = 0;
            RowData<SysRoleEntity> result = new RowData<SysRoleEntity>(_sysRoleService.GetList(param, out total));
            result.total = total;
            return result;
        }
        ...
        public ReturnData Save(RoleModel role)
        {
            ...
            _sysRoleService.Save(role.ToEntityCopy<RoleModel, SysRoleEntity>());
            // TODO 处理角色所属菜单
