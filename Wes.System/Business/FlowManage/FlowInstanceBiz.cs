...
namespace Wes.Business
{
    public class FlowInstanceBiz : IFlowInstanceBiz
    {
        ...
        {
            return new ResultData<FlowInstanceEntity>(_flowInstanceService.GeDetailById(id));
        }

        public ResultData<List<FlowInstanceEntity>> GetAll()
        {
            return new ResultData<List<FlowInstanceEntity>>(_flowInstanceService.GetAll());
        }

        public RowData<FlowInstanceEntity> GetList(ParamData<FlowInstanceParam> param)
        {
            int total = 0;
            RowData<FlowInstanceEntity> result = new RowData<FlowInstanceEntity>(_flowInstanceService.GetList(param, out total));
            result.total = total;
            return result;
        }
        ...
        public ResultData<FlowRunModel> Exec(FlowRunModel flowRunModel)
        {
            ...
            FlowInstanceEntity flowInstance = _flowInstanceService.GetByBusinessId(flowRunModel.ProcessCode, flowRunModel.BusinessId);
            ...
            try
            {
                ...
                if (flowRunModel.InstanceStatus == FlowStatusEnum.start)
                {
                    ...
                    flowInstance = new FlowInstanceEntity()
                    {
        ...
        public ReturnData TaskDelegate(long taskId, long userId)
        {
            ...
            var newTask = task.ToEntityCopy<FlowInstanceTaskEntity, FlowInstanceTaskEntity>();
