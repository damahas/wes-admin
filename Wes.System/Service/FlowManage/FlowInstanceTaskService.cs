...
{
    public class FlowInstanceTaskService : Repository<FlowInstanceTaskEntity>, IFlowInstanceTaskService
    {
        ...
        {
            Expressionable<FlowInstanceTaskEntity> express = Expressionable.Create<FlowInstanceTaskEntity>();
            ...
            total = 0;
            var query = Context.Queryable<FlowInstanceTaskEntity>().Where(express.ToExpression());
            if (param.PageSize == 0)
                return query.ToList();
        ...
        {
            var context = sqlSugarClient ?? Context;
            return context.Updateable<FlowInstanceTaskEntity>()
        ...
        public bool Delete(List<long> ids)
        {
            ...
            return Context.Updateable<FlowInstanceTaskEntity>().SetColumns(p => p.IsDel == 1)
