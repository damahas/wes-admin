...
namespace Wes.Service
{
    ...
    public class SyncDataSaveService : ISyncDataSaveService, ISyncDataAccess
    {
        ...
        public async Task<SyncSaveResult> SaveUsersAsync(string provider, List<SyncUserData> users)
        {
            ...
            foreach (var du in users)
            {
                try
                {
                    ...
                    {
                        var deptMapping = await _db.Queryable<SysThirdPartyMappingEntity>()
        ...
        {
            var mapping = await _db.Queryable<SysThirdPartyMappingEntity>()
        ...
        /// </summary>
        private async Task<SysDeptEntity> QueryDeptEntityByThirdPartyIdAsync(string thirdPartyId)
        {
            var mapping = await _db.Queryable<SysThirdPartyMappingEntity>()
            ...
            if (mapping != null && long.TryParse(mapping.LocalId, out var deptId))
                return await _db.Queryable<SysDeptEntity>().Where(d => d.DeptId == deptId && d.IsDel == 0).FirstAsync();
        ...
        /// </summary>
        private async Task<SysUserEntity> QueryUserEntityByThirdPartyIdAsync(string thirdPartyId)
        {
            var mapping = await _db.Queryable<SysThirdPartyMappingEntity>()
            ...
            if (mapping != null && long.TryParse(mapping.LocalId, out var userId))
                return await _db.Queryable<SysUserEntity>().Where(u => u.UserId == userId && u.IsDel == 0).FirstAsync();
        ...
        /// 根据三方 ID 获取或创建本地部门实体
        /// </summary>
        private async Task<SysDeptEntity> GetOrCreateDept(string provider, SyncDeptData dd)
        {
            var existing = await QueryDeptEntityByThirdPartyIdAsync(dd.ThirdPartyDeptId);
            return existing ?? new SysDeptEntity();
        }

        /// <summary>
        /// 根据三方 ID 获取或创建本地用户实体
        /// </summary>
        private async Task<SysUserEntity> GetOrCreateUser(string provider, SyncUserData du)
        {
            var existing = await QueryUserEntityByThirdPartyIdAsync(du.ThirdPartyUserId);
            return existing ?? new SysUserEntity();
        }

        /// <summary>
        /// 解析部门的父部门关系
        /// </summary>
        private static void ResolveDeptParent(SysDeptEntity dept, string thirdPartyParentId, Dictionary<string, long> mappingDict)
        {
        ...
        private async Task FixDeptAncestors(string provider, List<SyncDeptData> depts, Dictionary<string, long> mappingDict)
        {
            foreach (var dd in depts)
            {
                ...
                var parentMapping = await _db.Queryable<SysThirdPartyMappingEntity>()
                    .Where(m => m.Provider == provider && m.EntityType == "Dept" && m.LocalId == parentLocalId.ToString())
                    .FirstAsync();

                // 从父部门的完整 Ancestors 推导子部门的 Ancestors
                var parentDept = await _db.Queryable<SysDeptEntity>().Where(d => d.DeptId == parentLocalId).FirstAsync();
                ...
                await _db.Updateable<SysDeptEntity>()
        ...
        {
            var existing = await _db.Queryable<SysThirdPartyMappingEntity>()
            ...
            var mapping = new SysThirdPartyMappingEntity
            {
                Provider = provider,
