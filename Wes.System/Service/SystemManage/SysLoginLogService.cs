...
{
    public class SysLoginLogService : Repository<SysLoginLogEntity>, ISysLoginLogService
    {
        public SysLoginLogService(ISqlSugarClient db) : base(db) { }

        public bool Delete(List<long> ids)
        {
            return Context.Updateable<SysLoginLogEntity>().SetColumns(p => p.IsDel == 1)
                .Where(p => ids.Contains(p.LoginId)).ExecuteCommand() > 0;
        }

        public bool DeleteAll()
        {
            return Context.Updateable<SysLoginLogEntity>().SetColumns(p => p.IsDel == 1).ExecuteCommand() > 0;
        }

        public List<SysLoginLogEntity> GetList(ParamData<LoginLogParam> param, out int total)
        {
            Expressionable<SysLoginLogEntity> express = Expressionable.Create<SysLoginLogEntity>();
            ...
            total = 0;
            return Context.Queryable<SysLoginLogEntity>().Where(express.ToExpression())
