...
{
    public class SysUserService : Repository<SysUserEntity>, ISysUserService
    {
        ...
        {
            Expressionable<SysUserEntity> express = Expressionable.Create<SysUserEntity>();
            ...
            if (param.Params != null)
            {
                if (param.Params.RoleId > 0)
                {
                    ...
                    {
                        express.And(u => SqlFunc.Subqueryable<SysUserRoleEntity>().Where(s => s.RoleId == param.Params.RoleId && s.UserId == u.UserId).NotAny());
                    }
                    else
                    {
                        express.And(u => SqlFunc.Subqueryable<SysUserRoleEntity>().Where(s => s.RoleId == param.Params.RoleId && s.UserId == u.UserId).Any());
                    }
                }
            ...
            total = 0;
            var query = Context.Queryable<SysUserEntity>().OrderBy(p => p.UserId, OrderByType.Desc).Where(express.ToExpression());
            if (param.PageSize == 0)
            {
                return query.ToList();
            }
        ...
        {
            return Context.Queryable<SysUserEntity>()
                .InnerJoin<SysUserRoleEntity>((u, r) => u.UserId == r.UserId)
        ...
        {
            return Context.Queryable<SysUserEntity>()
                .InnerJoin<SysDeptEntity>((u, d) => u.DeptId == d.DeptId)
        ...
        public long GetLeaderIdByUserId(long userId)
        {
            ...
            var depts = Context.Queryable<SysDeptEntity>().Where(p => p.LeaderUserId > 0
        ...
        public List<SysUserEntity> GetList(ParamData<UserParam> param, out int total)
        {
            ...
            total = 0;
            var query = Context.Queryable<SysUserEntity>().Includes(p => p.Dept).Where(express.ToExpression());
            if (param.PageNum == 0)
                return query.ToList();
            return query.ToPageList(param.PageNum, param.PageSize, ref total);
        }
        ...
        {
            return Context.Queryable<SysUserEntity>().Where(p => userIds.Contains(p.UserId)).Includes(p => p.Dept).ToList();
        }
        ...
        {
            return Context.Updateable<SysUserEntity>().SetColumns(p => p.IsDel == 1)
        ...
        {
            return Context.Queryable<SysUserRoleEntity>().Where(p => p.UserId == userId).ToList();
        }
        ...
        {
            Expressionable<SysUserRoleEntity> express = Expressionable.Create<SysUserRoleEntity>();
            express.And(p => p.UserId == userId);
            if (roleIds != null && roleIds.Count > 0)
            {
                express.And(p => !roleIds.Contains(p.RoleId));
            }
            return Context.Deleteable<SysUserRoleEntity>().Where(express.ToExpression()).ExecuteCommand() > 0;
        }
        ...
        {
            return Context.Queryable<SysUserPostEntity>().Where(p => p.UserId == userId).ToList();
        }
        ...
        {
            Expressionable<SysUserPostEntity> express = Expressionable.Create<SysUserPostEntity>();
            express.And(p => p.UserId == userId);
            if (postIds != null && postIds.Count > 0)
            {
                express.And(p => !postIds.Contains(p.PostId));
            }
            return Context.Deleteable<SysUserPostEntity>().Where(express.ToExpression()).ExecuteCommand() > 0;
        }
        ...
        {
            return this.Context.Queryable<SysUserEntity>()
                .LeftJoin<SysTokenEntity>((o, p) => o.UserId == p.UserId)
        ...
        {
            return this.Context.Queryable<SysTokenEntity>().Where(p => p.Token == token && p.IsDel == 0 && p.Status == 0).First();
        }

        public bool InvalidToken(string token)
        {
            return Context.Updateable<SysTokenEntity>().SetColumns(p => p.Status == 1)
                    .Where(p => p.Token == token).ExecuteCommand() > 0;
        }

        public SysTokenEntity InvalidToken(long tokenId)
        {
            Context.Updateable<SysTokenEntity>().SetColumns(p => p.Status == 1)
                    .Where(p => p.TokenId == tokenId).ExecuteCommand();
            return Context.Queryable<SysTokenEntity>().Where(p => p.TokenId == tokenId).First();
        }

        public List<OnlineInfo> GetOnlineList(ParamData<OnlineParam> param, out int total)
        {
            Expressionable<SysTokenEntity, SysUserEntity, SysDeptEntity> express = Expressionable.Create<SysTokenEntity, SysUserEntity, SysDeptEntity>();
            ...
            total = 0;
            return Context.Queryable<SysTokenEntity, SysUserEntity, SysDeptEntity>((t, u, d) =>
                new JoinQueryInfos(
