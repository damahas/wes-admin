...
项目提供 Dockerfile，将前后端打包到同一镜像（Nginx 反向代理 + ASP.NET Core）：

```bash
# 1. 先执行前端 build 和后端 publish
cd Wes.UI && npm run build
cd ../Wes.WebApi && dotnet publish -c Release

# 2. 构建镜像（构建上下文 = 解决方案根目录）
cd ..
docker build -f Wes.WebApi/Dockerfile -t wes-admin .

# 3. 运行容器（需设置 ASP.NET Core 监听端口与 nginx 代理一致）
docker run -d -p 80:80 -e ASPNETCORE_URLS=http://+:8088 wes-admin
```

> nginx 监听 80 端口，反向代理 `/api/` 到 `localhost:8088`，前端静态文件由 `/front` 提供。
